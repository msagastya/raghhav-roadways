...
export default function ConsignorConsigneeMaster() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (editingId) {
        await mastersAPI.updateConsignorConsignee(editingId, formData);
        showSuccess('Updated successfully');
      } else {
        await mastersAPI.createConsignorConsignee(formData);
  ...
  const handleEdit = async (id) => {
    try {
      const response = await mastersAPI.getConsignorConsigneeById(id);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await mastersAPI.deleteConsignorConsignee(id);
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
        <div>
          <h3 className="text-base sm:text-lg font-semibold text-gray-900 dark:text-white">Consignor/Consignee Master</h3>
          <p className="text-xs sm:text-sm text-gray-600 dark:text-white/60">Manage consignor and consignee information</p>
        </div>
      ...
      <Modal
        ...
        title={editingId ? 'Edit Consignor/Consignee' : 'Add New Consignor/Consignee'}
      >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
            <div>
              ...
              <Select value={formData.stateId} onChange={handleStateChange}>
                ...
                {states.length === 0 && <option disabled>Loading states...</option>}
                {states.map((state) => (
              ...
              {states.length > 0 && <p className="text-xs text-gray-500 dark:text-white/60 mt-1">{states.length} states available</p>}
            </div>
            ...
            <div>
              ...
              >
                ...
                {cities.length === 0 && formData.stateId && <option disabled>Loading cities...</option>}
                {cities.map((city) => (
              ...
              {cities.length > 0 && <p className="text-xs text-gray-500 dark:text-white/60 mt-1">{cities.length} cities available</p>}
            </div>
          </div>
