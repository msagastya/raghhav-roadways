...
const LOAD_TYPES = ['FT L', 'FT S', 'FT M', 'LCL', 'Part Load', 'Half Load'];
...
export default function NewConsignmentPage() {
  ...
  const fetchConsignors = async () => {
    try {
      ...
      setConsignors(response.data?.data?.parties || []);
    } catch (error) {
  ...
  const fetchConsignees = async () => {
    try {
      const response = await mastersAPI.getConsignorConsignees({ limit: 100 });
      setConsignees(response.data?.data?.parties || []);
    } catch (error) {
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
              <div>
                ...
                <Input
                  ...
                  placeholder="e.g. Forklift, EV Charger, Reach Truck"
                  required
                />
              </div>
